...
Pantheon is written in Rust and uses Axum to do the heavy lifting on tasks like routing, subrouting, and extraction of parameters. It also provides a few ways to inject custom middleware. The `middleware::from_fn` and `middleware::from_fn_with_state` allow the generation of middleware by passing a function which resolves to a future, or some state and a function which resolves to a future respectively. That usage is relatively simple. For example:
...
However, we can't extract `roles` and we also don't have a way to parameterize it in its current form. Axum's `from_fn` and `from_fn_with_state` take the function you want to turn into middleware, not the result of that function. Basically, we need a function which returns a partially applied middleware. The way you can do this is as follows:
...
async fn main() -> Result<()> {
  ...
  let rbac = make_rbac(vec!["admin".to_string()]).await;

  let ctx = Context { /*...*/ };

  let app = Router::new()
      .route("/", get(|| async { /* ... */ }))
      .route_layer(middleware::from_fn_with_state(ctx.clone(), my_middleware))
      .with_state(ctx);
